...
- **Production-Safe**: Uses `--oplog`, `--readPreference=secondaryPreferred`, and `--numParallelCollections=1` for minimal database load
...
- **Comprehensive Logging**: Detailed logs with backup duration and status
...
Edit `docker-compose.yml` with your MongoDB connection details, or create a `.env` file.
...
# Then uncomment the build line in docker-compose.yml
```

### 3. Direct Docker Run

```bash
docker run -d --name mongodb-backup -e MONGO_CONNECTION_STRING="mongodb://127.0.0.1:27017/softgoods-prod" -e DAILY_BACKUP_INTERVAL_MINUTES=5 -v ./backups:/backups -p 3000:3000 mongo-backup:latest
```
...
- **Access Logs**: View detailed logs for failed backups
...
- **`--oplog`**: Captures point-in-time consistency
...
mongorestore --uri="mongodb://user:pass@host:27017" /path/to/backup/20250902_141500_MyDatabase/

# Restore specific collection
mongorestore --uri="mongodb://user:pass@host:27017" --collection=users /path/to/backup/20250902_141500_MyDatabase/MyDatabase/users.bson
```
